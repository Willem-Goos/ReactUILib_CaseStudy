...
export default function PiDropdown_Radix(props: PiSelectProps) {
    ...
    const options = values.map((value, index) => {
        return <DropdownMenu.Item className={"z-1000 hover:bg-pi-bluehover h-36 outline-none cursor-pointer flex align-middle"} key={value} onClick={() => onChange(index)}>
            <div className={"px-2 font-poppins my-auto"}>{value}</div>
        </DropdownMenu.Item>
    });

    return <DropdownMenu.Root onOpenChange={setIsOpen}>
        <DropdownMenu.Trigger className={"flex font-poppins box-border w-full px-2 border-2 rounded-full border-solid border-pi-blue h-36 focus:outline-none cursor-pointer" +
            " hover:bg-pi-bluehover z-40"}>
            <span className={"mt-auto mb-auto select-none"}>{values[selectedIndex]}</span>
